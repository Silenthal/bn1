...
Then, run `docker compose up`. The output GBA file will be in the `build` directory.

To run the rebuild command (tidy, then build), run `docker compose -f rebuild.yaml up`.

To clean up all build files, run `make clean`. Run `make tidy` to only remove built source files and leave built asset files alone. To remove the created container, use `docker compose down`. The created image is currently called `bn1-build` and can be removed afterwards with `docker rmi bn1-build`.
