...
SoundInfo gSoundInfo = {0};
MPlayFunc gMPlayJumpTable[36] = {0};
CgbChannel gCgbChans[4] = {0};
...
static void UnusedDummyFunc(void)
{
}
...
void m4aSoundInit(void)
{
    ...
    CpuCopy32((void *)((s32)SoundMain & ~1), SoundMain_Buffer, 0x400);
    ...
    for (i = 0; i < NUM_MUSIC_PLAYERS; i++)
    {
        ...
        MPlayOpen(mplayInfo, gMPlayTable[i].track, gMPlayTable[i].trackCount);
...
void m4aSongNumStart(u16 n)
{
    ...
    const struct MusicPlayer *mplay = &mplayTable[song->musicPlayer];
...
static void m4aSongNumStartOrContinue(u16 n)
{
    const struct MusicPlayer *mplayTable = gMPlayTable;
    const struct Song *songTable = gSongTable;
    const struct Song *song = &songTable[n];
...
static void m4aSongNumContinue(u16 n)
{
    const struct MusicPlayer *mplayTable = gMPlayTable;
    const struct Song *songTable = gSongTable;
    const struct Song *song = &songTable[n];
...
void ClearSoundChannelTracks(void *x)
{
#if __STDC_VERSION__ < 202311L
